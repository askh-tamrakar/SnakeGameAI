...
using static SnakeGameAI.NeuralNetwork;
...
namespace SnakeGameAI {
    public class AI_Debugger {
        ...
        private const float LayerMin = 20f, LayerMax = 400f;
        private const float NeuronMin = 2f, NeuronMax = 100f;
        private const float ZoomMin = 0.1f, ZoomMax = 3f;
        private const float RadiusMin = 5f, RadiusMax = 30f;
        private const float FontMin = 10f, FontMax = 40f;

        // ⚡ OPTIMIZATION: Inline Lerp method
        ...
        // ⚡ OPTIMIZATION: Static readonly JsonSerializerOptions
        ...
        public void SaveSettings() {
            string json = JsonSerializer.Serialize(displaySettings, JsonOptions);
        ...
        public void DrawFitnessGraph(Population population) {
            ...
            // Draw axis lines
            Raylib.DrawLine(axisX, graphY + margin, axisX, axisY, RayColor.Black);
            Raylib.DrawLine(axisX, axisY, graphX + graphWidth - margin, axisY, RayColor.Black);
            ...
            for(int i = 0; i <= xTicks; i++) {
                ...
                int gen = (int)(t * count);
                Raylib.DrawTextEx(font, $"{gen}", new Vector2(x - 10, axisY + 8), 15, 0, RayColor.Black);
            }
            ...
            for(int i = 0; i <= yTicks; i++) {
                ...
                double val = isLogScale
                    ? Math.Pow(10, Math.Log10(minFitness + 1e-5) + t * (Math.Log10(maxFitness + 1e-5) - Math.Log10(minFitness + 1e-5)))
            ...
            for(int i = 1; i < count; i++) {
                ...
                Raylib.DrawLineEx(new Vector2(x1, y1), new Vector2(x2, y2), 2.5f, RayColor.Blue);
            }
            ...
            Raylib.DrawLineEx(new Vector2((int)(bestX - crossSize), (int)(bestY - crossSize)),
                new Vector2((int)(bestX + crossSize), (int)(bestY + crossSize)), lineThickness, lineColor);
            Raylib.DrawLineEx(new Vector2((int)(bestX - crossSize), (int)(bestY + crossSize)),
                new Vector2((int)(bestX + crossSize), (int)(bestY - crossSize)), lineThickness, lineColor);
        ...
        // ⚡ OPTIMIZATION: Generic slider with better reuse
        public void DrawSlider(ref float value, float min, float max, int x, int y, string label, RayColor color) {
            const int width = 200;
        ...
        public void DrawSlider(ref int value, int min, int max, int x, int y, string label, RayColor color) {
            const int width = 200;
            Raylib.DrawText(label, x, y - 20, 20, RayColor.LightGray);
            Raylib.DrawRectangle(x, y, width, 10, RayColor.DarkGray);
            ...
            if(Raylib.IsMouseButtonDown(MouseButton.Left) && Raylib.CheckCollisionPointRec(mouse, barRect)) {
                float t = Math.Clamp((mouse.X - x) / width, 0f, 1f);
                int target = (int)(min + t * (max - min));
                value = (int)(value * 0.85f + target * 0.15f);
            }

            float knobX = x + (value - min) / (float)(max - min) * width;
            Raylib.DrawRectangle((int)knobX - 5, y - 5, 10, 20, color);
            Raylib.DrawText($"{value}", x + width + 15, y - 4, 18, color);
        }
        ...
        public void DrawDebugPanel(Genome genome, Population population, Vector2 panelOrigin,
            string genomeID, List<Genome>? bestGenomesEver = null) {
            ...
            int maxSteps = genome.Game.maxStepsWithoutFood + (genome.Score * 8);
            int stepsLeft = maxSteps - genome.Game.stepsSinceLastFood;
        ...
        public void DrawNeuralNetwork(Genome genome, Vector2 origin, int width, int height) {
            ...
            // ⚡ OPTIMIZATION: Pre-allocated list capacity
            var layerDense = new List<int>(genome.NeuralNetwork.GetLayers().Count + 1) {
            ...
            var OutputOfEachNeuron = new List<Matrix<double>> { inputMatrix };
            ...
            // ⚡ OPTIMIZATION: Pre-allocated positions list
            var positions = new List<Vector2[]>(layerDense.Count);
            ...
            for(int l = 0; l < layerDense.Count - 1; l++) {
                ...
                Matrix<double> weights = genome.NeuralNetwork.GetAllWeights()[l];

                for(int i = 0; i < weights.RowCount && i < currentLayer.Length; i++) {
                    for(int j = 0; j < weights.ColumnCount && j < nextLayer.Length; j++) {
        ...
        public static void DrawHeatmap(Matrix<double> matrix, int startX, int startY, int cellSize, double maxMutationCount = -1) {
            ...
            for(int i = 0; i < rows; i++) {
                for(int j = 0; j < cols; j++) {
                    ...
                    float intensity = (float)(value / maxMutationCount);
